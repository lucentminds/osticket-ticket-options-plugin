...
implements RestrictedAccess, Threadable {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postMessage($vars, $origin='', $alerts=true)
    {
        ...
        case !$vars['ip_address'] && $_SERVER['REMOTE_ADDR']:
        ...
        // Determines the message that was posted by the user.
        $message = $this->getThread()->addMessage( $vars, $errors );
        ...
        if ($cfg->alertONNewMessage()
            && ($email = $dept->getAlertEmail())
            && ($tpl = $dept->getTemplate())
            && ($o_tpl_message_alert = $tpl->getNewMessageAlertMsgTemplate())
        ) {
            $c_full_compiled_message = $this->replaceVars($o_tpl_message_alert->asArray(), $variables);
            ...
            foreach ($recipients as $k=>$staff) 
            {
                ...
                $alert = $this->replaceVars( $c_full_compiled_message, array('recipient' => $staff) );
                $email->sendAlert($staff, $alert['subj'], $alert['body'], null, $options);
                $sentlist[] = $staff->getEmail();
            }// /foreach()
            ...
            Signal::send('message-alert.sent', null, array(
                'subject' => $alert,
                'message_source' => $c_full_compiled_message,
                'options' => $options,
