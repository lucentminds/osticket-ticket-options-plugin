...
implements RestrictedAccess, Threadable, Searchable {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        /**********************************************************************/
        // <!-- Replaced by TicketOptionsPlugin -->
        if( $email && TicketOptionsPlugin::agent_context_email_enabled() )
        {
            // Create two mailing lists. One for staff and one for normal users.
            $recipient_staff_list = new MailingList();
            $recipient_user_list = new MailingList();

            /** 
             * Loop over each messages recipient and sort them as either staff
             * or user.
             */
            foreach( $recipients as $i => $o_recipient )
            {
                // Determines the recipient's e-mail address
                $c_email = $o_recipient->getContact()->getEmail()->email;

                /** 
                 * Determines a valid staff user object if the e-mail address
                 * matches a staff user. Returns null otherwise.
                 */
                $o_staff = Staff::lookup( $c_email );

                if( $o_staff )
                {
                    // This recipient is a staff member.
                    $recipient_staff_list->append( $o_staff );
                    continue;
                }

                // This recipient is a normal user.
                $recipient_user_list->append( $o_recipient );
            }
        }// /foreach()

        // Attempt to pull the message template.
        if( ( $tpl = $dept->getTemplate() ) 
            && ( $msg_tpl = $tpl->getReplyMsgTemplate() ) )
        {
            // Add ticket link (possibly with authtoken) if the ticket owner
            // is the only recipient on a ticket with collabs
            if (count( $recipient_user_list ) == 1
                && $this->getNumCollaborators()
                && ($contact = $recipient_user_list->offsetGet(0)->getContact())
                && ($contact instanceof TicketOwner))
            {
                $variables['recipient.ticket_link'] = $contact->getTicketLink();
            }
            ...
            // First send a message to normal users.
            $msg = $this->replaceVars($msg_tpl->asArray(),
                $variables + array('recipient' => $this->getOwner())
            );

            // Send normal user email.
            $email->send( $recipient_user_list, $msg['subj'], $msg['body'],
                $attachments, $options );




            // Next send a message to staff.
            $variables['recipient.ticket_link'] = $cfg->getBaseUrl()
                . '/scp/tickets.php?id='.$this->getId();
            ...
            // Send staff email.
            $email->send( $recipient_staff_list, $msg['subj'], $msg['body'],
                $attachments, $options );
        }// /if()
